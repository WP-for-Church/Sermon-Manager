...
class Sermon_Manager_Entry_Views{
  	...
  	*/
  	function __construct() {
			...
			add_shortcode( 'sermon-views', array( $this,'wpfc_entry_views_get') );
			/* Registers the entry views extension scripts if we're on the correct page. */
			add_action( 'template_redirect', array( $this,'wpfc_entry_views_load') );
			/* Add the entry views AJAX actions to the appropriate hooks. */
			add_action( 'wp_ajax_wpfc_entry_views', array( $this,'wpfc_entry_views_update_ajax') );
			add_action( 'wp_ajax_nopriv_wpfc_entry_views', array( $this,'wpfc_entry_views_update_ajax') );
		}
		...
		function wpfc_entry_views_load() {
			...
			if ( is_singular() ) {
				...
				if ( post_type_supports( $post->post_type, 'entry-views' ) ) {
					...
					add_action( 'wp_footer', array( $this,'wpfc_entry_views_load_scripts') );
				}
			}
		}
