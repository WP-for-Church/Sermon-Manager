 ...
 *
 */
 class Sermon_Manager_Shortcodes{
      ...
      add_shortcode('list_sermons', array( $this,'wpfc_list_sermons_shortcode') ); //preferred markup
      add_shortcode('list-sermons', array( $this,'wpfc_list_sermons_shortcode') ); //left for compatibility
      // Display all series or speakers in a grid of images
      add_shortcode('sermon_images', array( $this,'wpfc_display_images_shortcode') ); //preferred markup
      add_shortcode('sermon-images', array( $this,'wpfc_display_images_shortcode') ); //left for compatibility
      // Display the latest sermon series image (optional - by service type)
      add_shortcode( 'latest_series', array( $this,'wpfc_get_latest_series_image') );
      // Create the shortcode
      add_shortcode('sermons', array( $this,'wpfc_display_sermons_shortcode') );
      add_shortcode('sermon_sort_fields', array( $this,'wpfc_sermons_sorting_shortcode') );
  	}
    ...
    function wpfc_get_latest_series ( $latest_sermon = 0, $service_type = 0 ) {
    	if ( empty( $latest_sermon ) ) {
    		$latest_sermon = $this->wpfc_get_latest_sermon( $service_type );
    	}
    ...
    function wpfc_get_latest_series_image ( $atts ) {
    	...
    	$latest_sermon = $this->wpfc_get_latest_sermon( isset($service_type) );
    	$latest_series = $this->wpfc_get_latest_series( $latest_sermon );
    	$series_link = get_term_link( $latest_series, 'wpfc_sermon_series' );
    	$series_image_id = $this->wpfc_get_latest_series_image_id ( $latest_series );
    	...
    	$show_title = $this->wpfc_sanitize_bool( $show_title );
    	...
    	$show_desc = $this->wpfc_sanitize_bool( $show_desc );
    ...
    function wpfc_display_sermons_shortcode($atts) {
    	...
    	<div id="wpfc_sermon_wrap">
    		...
    		<?php $Sermon_Manager_Template_Tags = new Sermon_Manager_Template_Tags(); $Sermon_Manager_Template_Tags->wpfc_sermon_excerpt(); ?>
    	</div>
    	<?php
    	endwhile; //end loop ?>
    ...
    function wpfc_sermons_sorting_shortcode($atts) {
      ...
      $sorting = $Sermon_Manager_Template_Tags->render_wpfc_sorting();
    	return $sorting;
    }
}
$Sermon_Manager_Shortcodes = new Sermon_Manager_Shortcodes();
?>
