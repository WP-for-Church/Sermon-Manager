...
add_action( 'parse_request', function () {
	if ( sm_get_option( 'enable_podcast_redirection' ) ) {
		$old_url     = wp_make_link_relative( preg_replace( '{/$}', '', sm_get_option( 'podcast_redirection_old_url' ) ) );
		$current_url = preg_replace( '{/$}', '', $_SERVER['REQUEST_URI'] );

		if ( strpos( $current_url, $old_url ) !== false ) {
			wp_redirect( sm_get_option( 'podcast_redirection_new_url' ), 301 );
			exit;
		}
	}
} );
