<?php
class Sermon_Manager_Podcast_Functions{

 	 /**
 	 * Construct.
 	 */
 	 function __construct() {
		 add_action('pre_get_posts', array($this, 'wpfc_modify_sermon_date'), 9999);
		 add_action('do_feed_podcast', array($this, 'wpfc_sermon_podcast_feed'), 10, 1);
		 add_filter('generate_rewrite_rules', array($this, 'wpfc_sermon_podcast_feed_rewrite') );
 	 }
		...
		function wpfc_podcast_add_head(){
			...
			remove_filter('the_content', array($Sermon_Manager_Template_Tags, 'add_wpfc_sermon_content') );
      ...
      $Sermon_Manager_Template_Tags = new Sermon_Manager_Template_Tags();
			...
			<itunes:summary><?php wp_filter_nohtml_kses( $Sermon_Manager_Template_Tags->wpfc_sermon_meta('sermon_description') ); ?></itunes:summary>
			<?php if ( $post_image ) : ?>
		...
		function wpfc_podcast_summary ($content) {
			...
			//$content = '';
      $Sermon_Manager_Template_Tags = new Sermon_Manager_Template_Tags();
			$content = wp_filter_nohtml_kses( $Sermon_Manager_Template_Tags->get_wpfc_sermon_meta('sermon_description') );
				return $content;
		}
      ...
      $Sermon_Manager_Template_Tags = new Sermon_Manager_Template_Tags();
			return $Sermon_Manager_Template_Tags->wpfc_sermon_date('D, d M Y H:i:s O');;
		}
		...
		function wpfc_modify_sermon_date( $query ) {
			...
			if( is_post_type_archive('wpfc_sermon') || is_tax( 'wpfc_preacher' ) || is_tax( 'wpfc_sermon_topics' ) || is_tax( 'wpfc_service_type' ) || is_tax( 'wpfc_sermon_series' ) || is_tax( 'wpfc_bible_book' ) ) {
				add_filter ( 'get_post_time', array($this, 'wpfc_podcast_item_date'), 10, 3);
				add_filter('bloginfo_rss', array($this, 'wpfc_bloginfo_rss_filter') , 10, 2);
				add_filter( 'wp_title_rss', array($this, 'wpfc_modify_podcast_title') , 99, 3);
				add_action( 'rss_ns', array($this, 'wpfc_podcast_add_namespace') );
				add_action( 'rss2_ns', array($this, 'wpfc_podcast_add_namespace') );
				add_action('rss_head', array($this, 'wpfc_podcast_add_head') );
				add_action('rss2_head', array($this, 'wpfc_podcast_add_head') );
				add_action('rss_item', array($this, 'wpfc_podcast_add_item') );
				add_action('rss2_item', array($this, 'wpfc_podcast_add_item') );
				add_filter( 'the_content_feed', array($this, 'wpfc_podcast_summary'), 10, 3);
				add_filter( 'the_excerpt_rss', array($this, 'wpfc_podcast_summary') );
			}
			endif;
		}
